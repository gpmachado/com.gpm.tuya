...
const RETRY_THROTTLE_MS = 60 * 1000;
...
class IASZoneHelper {
  ...
  async init(zclNode) {
    ...
    if (this.readInitialState) {
      ...
      if (this._isEnrolled(attrs?.zoneState)) this._enrollConfirmed = true;
    }
  ...
  _installListeners() {
    ...
    this._onZoneStatusChangeNotification = payload => {
      ...
      this.retryOnWake('ias-report').catch(() => {});
    };
    ...
    if (typeof iasZone.on === 'function') {
      this._onAttrZoneStatus = zoneStatus => {
        ...
        this.retryOnWake('ias-attr').catch(() => {});
      };
  ...
  /**
   * Re-attempt enrollment when the device is known to be awake (any inbound
   * IAS frame). Sleepy end-devices may reject the CIE write / miss the enroll
   * response while asleep at app boot; this lets later wake-ups finish the job
   * without waiting for a full network rejoin (End Device Announce).
   * @param {string} source
   */
  async retryOnWake(source = 'wake') {
    if (this._enrollConfirmed || !this._iasZone) return;

    const now = Date.now();
    if (this._lastRetryTs && (now - this._lastRetryTs) < RETRY_THROTTLE_MS) return;
    this._lastRetryTs = now;

    const attrs = await this.readInitialStatus(source);
    if (this._isEnrolled(attrs?.zoneState)) {
      this._enrollConfirmed = true;
      this.device.log('[IAS] Enrollment confirmed on wake');
      return;
    }

    if (this.configureCieAddress) {
      const verified = await this.writeCieAddress();
      if (!verified) return;
    }

    await this.sendEnrollResponse(source);
  }
